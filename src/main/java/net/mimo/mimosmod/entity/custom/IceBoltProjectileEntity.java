...
public class IceBoltProjectileEntity extends Projectile {
    private static final EntityDataAccessor<Boolean> HIT =
            SynchedEntityData.defineId(IceBoltProjectileEntity.class, EntityDataSerializers.BOOLEAN);
    private int counter = 0;
    public IceBoltProjectileEntity(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }
    public IceBoltProjectileEntity(Level pLevel, Player player) {
        super(ModEntities.ICE_BOLT_PROJECTILE.get(), pLevel);
        setOwner(player);
    ...
    protected void onHitEntity(EntityHitResult hitResult) {
        ...
        if (hurt) {
            if(hitEntity instanceof LivingEntity livingHitEntity) {
                livingHitEntity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 40, 1), owner);
            }
        }
    }
